...
namespace GSRSExcelTools.Utils
{
    public class SheetUtils
    {
        ...
        public async Task CreateSheet(Workbook workbook, ScriptUtils scriptUtils,
            IScriptExecutor scriptExecutor, bool sortAlpha)
        {
            ...
            int numberOfRows = await GetNumberOfRows(scriptExecutor);
            Worksheet nsheet;
            int i;
            ...
            for (i = 0; i < argListLength; i++)
            {
                ...
                object argNameRaw = await scriptExecutor.ExecuteScript("tmpScript.arguments[" + i + "].name");
                string argName = TextUtils.StripQuotes((string)argNameRaw);
                cell.FormulaR1C1 = argName;
                string argDescription = (string) await scriptExecutor.ExecuteScript("tmpScript.arguments[" + i + "].description");
                object usedForLookupValue = await scriptExecutor.ExecuteScript("tmpScript.arguments[" + i + "].usedForLookup");
                ...
                object argTypeRaw = await scriptExecutor.ExecuteScript("tmpScript.arguments[" + i + "].type");
                cell.ColumnWidth = 21;
                ...
                string vocabularyName = await scriptUtils.GetVocabName(i);
                log.DebugFormat("in CreateSheet, got vocabularyName: {0}", vocabularyName);

                if( string.IsNullOrWhiteSpace(vocabularyName) 
                    && argTypeRaw != null )
                {
                    argTypeRaw = TextUtils.StripQuotes(argTypeRaw.ToString());
                    if(argTypeRaw.ToString().Equals("boolean", StringComparison.InvariantCultureIgnoreCase))
                    {
            ...
            Worksheet worksheet, int firstPart = 1)
        {
            for (int part = firstPart; part < data.Length; part++)
            {
                ...
                else if(result.StartsWith(ImageOps.jsChemifyImageStart))
                {
                    Range currentCell = worksheet.Range[cellId];
                    if( ImageOpsHandle == null)
                    {
                        ImageOpsHandle = new ImageOps();
                    }
                    ImageOpsHandle.InsertImageFromBase64(currentCell, result, 240);
                }
                else
                {
                    Range currentCell = worksheet.Range[cellId];
        ...
        public async void SetupRemainingColumns(Worksheet worksheet,
            IScriptExecutor scriptExecutor, ScriptUtils scriptUtils = null)
        {
            ...
            await ScriptExecutor.ExecuteScript("tmpScript=Scripts.get('" + SDFileProcessor.SD_LOADING_SCRIPT_NAME
                   + "');");
            object lengthRaw = await ScriptExecutor.ExecuteScript("tmpScript.arguments.length");
            int argListLength = Convert.ToInt32(lengthRaw);
            for (int i = 0; i < argListLength; i++)
            {
                ...
                if (requiredParms.Contains(argName) && !columnHeaders.Contains(argName))
                {
                    ...
                    string vocabularyName = await scriptUtils.GetVocabName(i);
                    log.DebugFormat("in SetupRemainingColumns, got vocabularyName: {0}", vocabularyName);
                    if(!string.IsNullOrWhiteSpace(vocabularyName))
                    {
        ...
        private async Task<int> GetNumberOfRows(IScriptExecutor scriptExecutor)
        {
            object numberOfRowsObj = await scriptExecutor.ExecuteScript("$('#numberOfRows').val()");
            log.DebugFormat("numberOfRowsObj : {0}", numberOfRowsObj);
            if (numberOfRowsObj != null)
            {
                try
                {
                    numberOfRowsObj = TextUtils.StripQuotes(numberOfRowsObj.ToString().Trim());
