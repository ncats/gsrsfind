...
namespace GSRSExcelTools.Utils
{
    public class FileUtils
    {
        ...
        {
            string javascriptFilePath = GetCurrentFolder();
            if (!javascriptFilePath.EndsWith(@"\"))
            {
                javascriptFilePath += @"\";
            }
            javascriptFilePath  += @"etc\gsrs_controller.js";
        ...
        public static string GetLastJavaScript()
        {
            ...
            javascriptFilePath += @"etc\LastScript.js";
            return File.ReadAllText(javascriptFilePath);
        }
        ...
        public static string GetChemifyJavaScript()
        {
            string javascriptFilePath = GetCurrentFolder() + @"etc\chemifyPiece.js";
            return File.ReadAllText(javascriptFilePath);
        }
        ...
        {
            string styleFilePath = GetCurrentFolder() + @"\etc\gsrs_controller.css";
            log.DebugFormat("looking for css file at {0}", styleFilePath);

            return File.ReadAllText(styleFilePath);
        }

        public static string GetEtcFileContents(string fileName)
        {
            string styleFilePath = GetCurrentFolder() + @"etc\" + fileName;
            return File.ReadAllText(styleFilePath);
        }   
        ...
        public static string LogTextToTemporaryFile(string text)
        {
            string filePath = GetTemporaryFilePath("log");
        ...
        public static string GetJSChemify()
        {
            string javascriptFilePath = GetCurrentFolder();
            if (!javascriptFilePath.EndsWith(@"\"))
            {
            ...
            javascriptFilePath += @"etc\jschemify.js";
            return File.ReadAllText(javascriptFilePath);
        }
