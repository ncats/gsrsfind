...
namespace GSRSExcelTools.Controller
{
    public class DataLoader : ControllerBase, IController
    {
        ...
        /// </summary>
        public async void ContinueSetup()
        {
            ...
            _scriptName = TextUtils.StripQuotes( _scriptName);
            scriptUtils.ScriptName = _scriptName;
            await SetScriptParametersAsync(ExcelSelection.Application.ActiveCell);
            ...
            UpdateCallback cb = await CreateInitialUpdateCallback(ExcelSelection.Application.ActiveCell, true);
            if (!string.IsNullOrEmpty(cb.SetupMessage)) StatusUpdater.UpdateStatus(cb.SetupMessage);
            else
            {
                log.DebugFormat("Total selected rows: {0}", totalSelectedRows);
            }
            if (cb != null) await ScriptExecutor.ExecuteScript("showPreview(tmpRunner)");
        }

        public async Task<bool> StartResolution(bool newSheet)
        {
            if (CurrentOperationType == OperationType.ShowScripts)
            {
                string selectedScriptName = (string) await ScriptExecutor.ExecuteScript("$('#scriptlist').val()");
                selectedScriptName = TextUtils.StripQuotes(selectedScriptName);
                log.DebugFormat("{0} selected script name {1}", MethodBase.GetCurrentMethod().Name,
                    selectedScriptName);
                ...
                if(scriptUtils.ExpectedVocabularies.Count==0)
                {
                    ...
                    await sheetUtils.CreateSheet(ExcelWindow.Application.ActiveWorkbook, scriptUtils,
        ...
        private async void StartLoading(Excel.Range r)
        {
            _totalScripts = 0;
            ...
            foreach (Excel.Range row in r.Rows)
            {
                ...
                Callback cb = await CreateUpdateCallbackForExecution(row);
                if( cb==null)
                {
            ...
            log.DebugFormat("in StartLoading, expected vocabularies count is {0}",
                scriptUtils.ExpectedVocabularies.Count);
            if (scriptUtils.ExpectedVocabularies.Count == 0)
            {
        ...
        public async Task<Callback> CreateUpdateCallbackForExecution(Excel.Range arow)
        {
            string msg = string.Empty;
            Callback cb = await CreateInitialUpdateCallback(arow);
            if (cb == null)
            {
        ...
        public async Task<UpdateCallback> CreateInitialUpdateCallback(Excel.Range arow,
            bool allowFinished = false)
        {
            ...
            try
            {
               if(ColumnKeys==null) ColumnKeys= GetColumnKeys(arow);
                ...
                {
                    log.DebugFormat("Looking at ScriptParameters.Keys {0}", key);
            ...
            updateCallback.SetupMessage = message;
        ...
        public async Task<object> HandleResults(string resultsKey, string message)
        {
        ...
        new public async Task ReceiveVocabulary(string rawVocab)
        {
            ...
            string cleanVocabName = TextUtils.StripQuotes(vocabName);
            rawVocab = rawVocab.Substring(delim2 + 1);
            Vocab vocab = JSTools.GetVocabFromString(rawVocab);

            scriptUtils.Vocabularies.Add(cleanVocabName, vocab);
            scriptUtils.MarkVocabArrived(cleanVocabName);
        ...
        private async Task SetScriptParametersAsync(Excel.Range range)
        {
            ...
            await ScriptExecutor.ExecuteScript(tempScriptName + "=Scripts.get('" + _scriptName + "');");
            string runnerName = "tmpRunner";
            await ScriptExecutor.ExecuteScript(runnerName + "=" + tempScriptName + ".runner();");

            await scriptUtils.BuildScriptParameters(keys.Keys);
        }
