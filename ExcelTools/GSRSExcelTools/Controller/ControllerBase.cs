...
namespace GSRSExcelTools.Controller
{
    public class ControllerBase : IDisposable
    {
        ...
        public async Task ReceiveVocabulary(string rawVocab)
        {
        ...
        protected async void SaveAndClearDebugInfo()
        {
            log.Debug("Starting in SaveAndClearDebugInfo");
            string fileName = FileUtils.GetTemporaryFilePath("gsrs.excel.log");
            string content = (string) await ScriptExecutor.ExecuteScript("GSRSAPI_consoleStack.join('|')");
            FileUtils.WriteToFile(fileName, content);
            await ScriptExecutor.ExecuteScript("GSRSAPI_consoleStack=[]");//clear the old stuff
        }
