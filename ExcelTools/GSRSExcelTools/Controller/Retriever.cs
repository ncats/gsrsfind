    ...
    public class Retriever : ControllerBase, IController
    {
        ...
        public async void StartOperation()
        {
            ...
            if (searchValues.Any(v => !string.IsNullOrWhiteSpace(v.Value)))
            {
                ...
                await ScriptExecutor.ExecuteScriptNoReturn(searchScript);
            }
            else
            {
        ...
        public async Task<bool> StartResolution(bool newSheet)
        {
            ...
            if (newSheet)
            {
                ...
                wrapped = await GetNewSheetResolverCursor();
            }
        ...
        private string MakeImageSearch_new(string key, List<string> names)
        {
            StringBuilder scriptBuilder = new StringBuilder();
            scriptBuilder.Append("cresults['");
            scriptBuilder.Append(key);
            ...
            //scriptBuilder.Append("'.split('\n'))");
            scriptBuilder.Append(")");
            scriptBuilder.Append(".fetchers(['SMILES+'])");
            scriptBuilder.Append(".consumer(function(row){console.log('row: '+JSON.stringify(row));");
            scriptBuilder.Append("var chemData = row.split('\t')[1];");
            scriptBuilder.Append("console.log('$: ' + $);");
            scriptBuilder.Append("console.log('_: ' + _);");
            //scriptBuilder.Append("console.log('jsonpatch: ' + jsonpatch);");
            //scriptBuilder.Append("console.log('JSChemify: ' + JSON.stringify(JSChemify));");
            //scriptBuilder.Append("for(int i = 0; i< document.getElementsByTagName('script').length; i++) {");
            //scriptBuilder.Append("  var scrTxt = document.getElementsByTagName('script')[i].innerText;");
            //scriptBuilder.Append("  console.log('scrTxt: ' + scrTxt.substring(0, Math.min(0, scrTxt.length));");
            //scriptBuilder.Append("}");
            //scriptBuilder.Append("var chem = JSChemify.Chemical(chemData);");
            scriptBuilder.Append("cresults['");
            scriptBuilder.Append(key);
            scriptBuilder.Append("'].add(row.split('\t')[0],row);})");
            scriptBuilder.Append(".finisher(function(){sendMessageBackToCSharp('");
        ...
        public void LaunchCheckJob()
        {
            ...
            _timer = new System.Timers.Timer(_checkInterval * secondsToMilliseconds);
        ...
        private async Task<RangeWrapper> GetNewSheetResolverCursor()
        {
            string[] headers;
            object checkedInput = await ScriptExecutor.ExecuteScript("_.map($('div.checkop input:checked'), 'name').join('___');");
            string[] splitters = { "___" };
            string cleanedHeaders = TextUtils.StripQuotes(checkedInput as string);
            headers = cleanedHeaders.Split(splitters, StringSplitOptions.None);
        ...
        public Dictionary<string, Callback> GetCallbacks()
        {
            return Callbacks;
        }
