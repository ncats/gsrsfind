...
var GSRSAPI = {
        ...
        g_api.GlobalSettings = {
            ...
            console.log('httpProcess req: ' + JSON.stringify(req));
                return g_api.JPromise.of(function (cb) {
                    ...
                    console.log('b128: ' + JSON.stringify(b));
                    $.ajax({
                        url: req._url,
                        /*jsonp: cbackname,*/
                        ...
                        beforeSend: function (request) {
                            console.log('beforeSend for ' + req._url + ' at ' + (new Date(_.now())));
                            if (req.headers) {
    ...
    extensions: [],
    setImageWidth: function (newWidth) {
        imageWidth = newWidth;
    },
    setImageHeight: function (newHeight) {
...
FetcherRegistry.addFetcher(
    FetcherMaker.make("Active Moiety ID", '', function (simpleSub) {
        return simpleSub.fetch("relationships")
            .andThen(function (r) {
                console.log('in active moiety id, r: ' + JSON.stringify(r));
                return _.chain(r)
                    .filter({ type: "ACTIVE MOIETY" })
                    .map(function (ro) {
...
FetcherRegistry.addFetcher(
    FetcherMaker.make("InChI", "InChI", function (simpleSub) {
        return simpleSub.fetch("structure!$inchi()")
            .andThen(function (ik) {
                console.log('in InChI resolver, id: ' + ik);
                if (!ik) return null;
                if (typeof ik === 'object') {
                    if (ik.retMsg) {
                        return ik.retMsg;
    ...
    FetcherMaker.make("Molfile+", "Molfile_PLUS", function (simpleSub) {
        return simpleSub.fetch("structure/molfile")
            .andThen(function (s) {
                ...
                return 'this is your ' + molfile;
            });
    }).addTag("Chemical")
);

/*FetcherRegistry.addFetcher(
    FetcherMaker.make("ImageFromMol", "Molfile_Image", function (simpleSub) {
        var imageToReturn = '';
        var innerResultId = UUID.randomUUID();
        var promiseToResolve = null;
        return simpleSub.fetch("structure/molfile")
            .andThen(async function (s) {
                if (s && (s.status === 404 || s.valid === false || jQuery.isEmptyObject(s))) {
                    console.log("No structure found for substance.  Will look at related");
                    return simpleSub.fetch("relationships")
                        .andThen(function (r) {
                ...
                imageToReturn = 'some intermediate value';
                
                console.log('retrieved molfile: ' + molfile);
                if (molfile && molfile !== null && molfile.length > 0) {
                    let chemical = JSChemify.Chemical(molfile);
                    imageToReturn = 'within next code block';
                    console.log(`before inner promise, keys: ${_.keys(window.innerResults)}`);
                    JSChemify.Renderer().getPNGPromise(chemical, imageWidth, imageHeight)
                        .then(function (image) {
                            console.log('   size of image: ' + image.length + ' type ' + typeof image);
                            let resultCommand = window.innerResults[innerResultId] = image;
                            console.log(`at end of inner promise, resultCommand: ${resultCommand}. keys: ${_.keys(window.innerResults)}`);
                            imageToReturn = image;
                            //return image;
                        })
                        .catch(function (err) {
                            console.log("Error retrieving result")
                            console.log('promResult: ' + JSON.stringify(promResult));
                        });
                }
                //imageToReturn = window.innerResults[innerResultId];
                console.log('returning results based on innerResultId: ' + innerResultId);
                return "{'innerResultId': '" + innerResultId + "'}";
            });
    }).addTag("Chemical Exp")
);* /
...
FetcherRegistry.addFetcher(
    FetcherMaker.make("Record URL", "Record_URL", function (simpleSub) {
        return JPromise.ofScalar(GlobalSettings.getHomeURL() + "ui/substances/" + simpleSub.uuid);
    }).addTag("Record")
);
...
Script.builder().mix({ name: "Add Name", description: "Adds a name to a substance record" })
    ...
    .setExecutor(function (args) {
        ...
        var nameObject = Name.builder().setName(name)
            ...
            .setLanguages(langs);
        if (args.standardize.isYessy()) {
            nameObject = nameObject.standardize(name);
        }
                            ...
                            return substance.fetch("references")
                                .andThen(function (refs) {
                                    ...
                                    if (totalCodesForSystem === 0) {
                                        ...
                                        console.log('more than 1 code for system found. No change will be made');
                                        return {
...
Script.builder().mix({ name: "Create Substance", description: "Creates a brand new substance record" })
    ...
    .addArgument({
        "key": "pd", name: "PD",
        description: "Public Domain status of the substance (sets access for reference as well)",
        defaultValue: false, required: false, type: "boolean"
    })
    .addArgument({
        "key": "cas", name: "CAS",
        description: "CAS Registry number (primary) for  the new substance",
        defaultValue: null, required: false
    })
    .addArgument({
        "key": "casCodeSystem", name: "CAS CODE SYSTEM",
        description: "Code System for CAS number",
        defaultValue: "CAS", required: false
    })
    .addArgument({
        "key": "change reason", name: "CHANGE REASON",
    ...
    .setExecutor(function (args) {
        ...
        var code = null;
        var cas = args.cas.getValue();
        var codeSystem = args.casCodeSystem.getValue();
        console.log("codeSystem  " + codeSystem);
        if (cas && cas != null && cas.length > 0) {
            var codeType = "PRIMARY";
            var url = "https://commonchemistry.cas.org/detail?cas_rn=" + cas;
            code = Code.builder().setCode(cas)
                .setType(codeType)
                .setCodeSystem(codeSystem)
                .setPublic(dataPublic)
                .setUrl(url);
            /*We may add comments and text later...
             * if (codeComments) {
                code.setCodeText(codeComments);
            }
            if (codeText) {
                ...
                for (var i = 0; i < s.names.length; i++) {
                    if (s.names[i].name === nameToReplace) {
                        ...
                        name = Name.builder().setName(newName)
                            ...
                            .setPreferred(s.names[i].preferred)
                            .setDisplay(s.names[i].displayName)
