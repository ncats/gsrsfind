...
namespace GSRSExcelTools.Model
{
    public class ImageOps
    {
        ...
        public static string jsChemifyImageStart = "data:image/png;base64,";
        ...
        {
            if( cell == null || cell.Comment == null) return false;
            try
            {
        ...
        public async Task CreateMolfileImage(Range cell, string molfile, Range cellForId)
        {
            string cleanMolfile = molfile.Replace("\r", "");
            string structureImageUrl = string.Empty;
            try
            {
                await RestUtils.SaveMolfileAndDisplay(cleanMolfile, cell, configuration.SelectedServer.ServerUrl, cellForId);
            }
            catch (Exception ex)
            {
        ...
        public void InsertImageFromBase64(Range cell, string base64Image, int size)
        {
           
            if (cell == null || string.IsNullOrEmpty(base64Image))
            {
                log.Error("Invalid cell or image data.");
                return;
            }
            try
            {
                cell.Select();
                string cleanBase64Image = base64Image.Replace(jsChemifyImageStart, string.Empty);
                byte[] imageBytes = Convert.FromBase64String(cleanBase64Image);
                Image image;
                using (MemoryStream ms = new MemoryStream(imageBytes))
                {
                    image = Image.FromStream(ms, true);
                }
                string tempFilePath = GetTempFile("png");
                image.Save(tempFilePath);
                log.DebugFormat("wrote image to file {0}", tempFilePath);
                // Insert the image (returns a Shape object)
                Shape picture = cell.Worksheet.Shapes.AddPicture(
                    tempFilePath,
                    Microsoft.Office.Core.MsoTriState.msoFalse, // Don't link to file
                    Microsoft.Office.Core.MsoTriState.msoCTrue, // Save with the document
                    (float)cell.Left,
                    (float)cell.Top,
                    (float)cell.Width,
                    (float)cell.Height
                ...
                // Optionally resize to fit the cell precisely
                picture.LockAspectRatio = Microsoft.Office.Core.MsoTriState.msoFalse;
                picture.Top = (float)cell.Top;
                picture.Left = (float)cell.Left;
                picture.Width = (float)cell.Width;
                picture.Height = (float)cell.Height;
            }
            catch (Exception ex)
            {
                log.Error($"Error inserting image: {ex.Message}");
            }
        }
    }
