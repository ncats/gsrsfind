...
namespace GSRSExcelToolsTests.utils
{
    internal class MySQLDBUtils
    {
        ...
        internal List<Tuple<string, string>> GetCodesForCode(string codeToQuery, string codeSystemToQuery)
        {
            string query =
                string.Format("select code_system, code from ix_ginas_code where owner_uuid in (select owner_uuid from ix_ginas_code where code='{0}' and code_system = '{1}') and code_system != '{1}'",
                codeToQuery, codeSystemToQuery);
