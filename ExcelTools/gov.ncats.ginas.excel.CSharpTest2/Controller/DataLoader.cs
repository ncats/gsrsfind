...
using gov.ncats.ginas.excel.CSharpTest2.Providers;
using System.Timers;
...
namespace gov.ncats.ginas.excel.CSharpTest2.Controller
{
    public class DataLoader : ControllerBase, IController
    {
            ...
            string message = "")
        {
            ...
            //todo: once we confirm that the current method of setting values is working, delete the gSetValue
            // function
            //_scriptExecutor.ExecuteScript(
            //    "tmpRunner.gSetValue=function(s){tmpRunner.setValue(s.split('|_|')[0],s.split('|_|')[1]);}");
                ...
                if (!string.IsNullOrWhiteSpace(GetProperty(keys, key, "")))
                {
                            ...
                            StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        string parameterValue = (keys[key].Text != null) ? keys[key].Text :
                            string.Empty;
                        parameterValue = parameterValue.Replace("'", "\\'").Replace(Environment.NewLine, 
                            "\\" + Environment.NewLine);
                        ...
                        //WebBrowser1.Document.script.tmpRunner.gSetValue(k & "|_|" & s)
                    }
                }
            }
